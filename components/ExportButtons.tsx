...
export default function ExportButtons({ session }: { session: Session }) {
  ...
  return (
    <button
      ...
      className="inline-flex min-h-11 items-center justify-center rounded-md border border-zinc-300 bg-white px-4 text-sm font-semibold text-zinc-800 shadow-sm hover:bg-zinc-50 disabled:cursor-not-allowed disabled:text-zinc-400 disabled:shadow-none"
    >
      Export CSV
    </button>
  );
